...
Original file is located at
    https://colab.research.google.com/drive/1p8vnY_pAETMXVt1RgN1KacPOMqp0rmSP
"""
...
def upload_excel():
    try:
        ...
        # Running locally - prompt for path
        fname = input("Enter path to 'Traditional-Format-Input.xlsx': ").strip()
        return fname

def read_bs_and_pl(iofile):
    xl = pd.ExcelFile(iofile)

    # Find header row in Balance Sheet (look for 'LIABILITIES')
    ...
    # Find header row in Profit & Loss (look for 'DR.PATICULARS')
...
# ===============================
# 4. Mapping BALANCE SHEET figures
# ===============================
...
authorised_cap = max(share_cap_cy, share_cap_py) * 1.2  # 20% buffer over highest amount
...
surplus_open_cy = surplus_py  # Opening is prior year closing
surplus_open_py = 70000      # Prior year opening balance
...
longterm_prov_cy = 0  # No data available
longterm_prov_py = 0

# Short-term borrowings (Note 6)
shortterm_borrow_cy = 0  # No data in input
...
dta_cy = 0  # No data available
dta_py = 0

# Long-term Loans and Advances (Note 14)
longterm_loans_cy = 0  # No data available
longterm_loans_py = 0
...
current_inv_cy = 0  # No data available
current_inv_py = 0
...
# Calculate total balances for verification
...
# Calculate shares for EPS
num_shares = share_cap_cy / 10 if share_cap_cy > 0 else 10000  # Assuming ₹10 per share
...
# Note 11: Fixed Assets - Tangible Assets
note11 = pd.DataFrame({
    'Asset Class': [
        'Land & Building',
        'Plant & Machinery',
...
# Note 13: Capital Work-in-Progress
note13 = pd.DataFrame({
...
# ===============================
# 7. Write Excel with all 26 notes
# ===============================
...
outfile = "Schedule_III_Complete_Output.xlsx"
...
with pd.ExcelWriter(outfile, engine='openpyxl') as writer:
    ...
    # Write all notes in separate sheets for better organization
    write_notes_with_labels(writer, "Notes 1-5", [
    ...
    write_notes_with_labels(writer, "Notes 6-10", [
    ...
    write_notes_with_labels(writer, "Notes 11-15", [
    ...
    write_notes_with_labels(writer, "Notes 16-20", [
    ...
    write_notes_with_labels(writer, "Notes 21-26", [
...
print(f"\n✅ Complete Schedule III Output with all 26 Notes saved as: {outfile}")
print(f"📊 Balance Sheet Total: Assets = ₹{total_assets_cy:,.0f}, Liabilities = ₹{total_equity_liab_cy:,.0f}")
print(f"💰 P&L Summary: Revenue = ₹{total_rev_cy:,.0f}, PAT = ₹{pat_cy:,.0f}")
print(f"📈 EPS: Current Year = ₹{eps_cy:.2f}, Previous Year = ₹{eps_py:.2f}")

# For Google Colab, optionally enable download:
try:
    from google.colab import files
    files.download(outfile)
except ImportError:
    print(f"Check your current directory for the file: {outfile}")